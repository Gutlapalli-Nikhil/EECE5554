...
figure;
plot(imu_time, vel_imu, "DisplayName","Acceleration (Forward Velocity)",'LineWidth',2.0);
hold on;
plot(gps_time, vel_gps, "DisplayName"," Velocity obtained from GPS ",'LineWidth',2.0);
xlabel('time (s)')
ylabel('vel (m/s)')
title('Accelerometer (Forward velocity) vs Velocity from GPS')
legend;
...
figure;
plot(imu_time,acc_x,"DisplayName"," Before Adjusting", 'LineWidth',2.0);
hold on;
plot(imu_time,accel_correct,"DisplayName","After Adjusting", 'LineWidth',2.0);
xlabel('time (s)')
ylabel('accel (m/s^2)')
title('Acceleration before - after removing of bias')
legend


figure;
plot(gps_time,vel_gps,"DisplayName"," Velocity estimate from GPS ", 'LineWidth',2.0);
hold on;
plot(imu_time,fwd_correct_vel,"DisplayName","Integrated forward velocity (Accel) - after adjusting", 'LineWidth',2.0);
xlabel('time (s)')
ylabel('vel (m/s)')
